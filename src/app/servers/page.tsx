...
import { getAssociatedServers } from "@/api/account/servers";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setServers(getServersVinculated.servers);
          ...
          <div className="empty-table-message mb-4 select-none">
            <div className="content mb-30 mt-16">
              ...
              <p className="mb-5 font-serif">
                No fue posible obtener los servidores asociados
              </p>
              <LoadingSpinner />
            </div>
          </div>
    ...
    <div className="contenedor dark h-screen-md select-none ">
      ...
      <div className="text-center pt-32">
        <h1 className="text-4xl font-bold text-white">
          {t("server-accounts.title")}
        </h1>
        <p className="mt-4 text-xl text-gray-300 max-w-3xl mx-auto">
          {t("server-accounts.subtitle")}
        </p>
      </div>
      {hasAccount ? (
        <div className="relative shadow-md sm:rounded-lg pt-16">
            ...
            <div className="relative inline-block text-left ml-2">
              ...
              >
                {!accountMaximus && (
                  ...
                  >
                    <li>
                      ...
                      >
                        {t("server-accounts.btn.create-server")}
                      </Link>
                    </li>
                  </ul>
                )}
                <div className="py-1">
                  ...
                  >
                    {t("server-accounts.btn.delete-server")}
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="max-h-[400px] overflow-y-auto min-h-[400px] flex flex-col justify-between">
              ...
              <thead className="text-lg text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-id")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-name")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-expansion")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-status")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-date")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-apikey")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                    {t("server-accounts.table.column-web")}
                  </td>
                  <td scope="col" className="px-6 py-3">
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <a
                        ...
                        onClick={() =>
                          router.push(`/servers/dashboard?id=${row.id}`)
                        }
                      >
            ...
            <div className="flex justify-center items-center mt-10 ">
              <ReactPaginate
                previousLabel={t("server-accounts.btn.paginate-btn-secondary")}
                nextLabel={t("server-accounts.btn.paginate-btn-primary")}
                breakLabel={""}
        ...
        <div className="empty-table-message items-center justify-center">
          <div className="content shadow-md sm:rounded-lg select-none">
            ...
            {user.pending_validation && (
              <button
                className="w-full sm:w-1/2 bg-blue-600 text-white px-5 py-3 rounded-md hover:bg-blue-700 transition"
                onClick={handleConfirmEmail}
              >
                {t("account.without-accounts.confirm-mail.btn-txt")}
              </button>
            )}
            {!user.pending_validation && servers && servers.length <= 10 && (
              <Link
                className="w-full sm:w-1/2 bg-blue-600 text-white px-5 py-3 rounded-md hover:bg-blue-700 transition"
                href="/register/server"
              >
